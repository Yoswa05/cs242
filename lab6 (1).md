...
> Your answer:The value at h[1] is always the maximum value currently in the heap. The heap property guarantees that every parent is greater than or equal to its children. Since the root is the ancestor of every node, no value can be larger than h[1].
...
> Your answer: 20 is inserted at index 5.
...
> Your answer: A complete binary tree must fill levels from left to right with no gaps. Removing any other node would create holes inside the tree. The last array element corresponds to the deepest rightmost node, so moving it to the root keeps the structure valid.
...
> Your answer: During sift-down, an element can move down at most one level per swap, so the maximum possible swaps equals the tree height.
...
> Your answer: The ratio becomes more stable as n grows because vector overallocation affects small arrays more strongly.
...
> Your answer: The CPU loads nearby values together into cache lines, so accessing h[i] and h[i/2] is very fast.
...
> Your answer: A linked-node heap is useful when the heap size changes unpredictably,contiguous memory is unavailable nodes must be linked into other structures simultaneously.
...
> Your answer: Indices that n/2 through n−1 are leaf nodes. Leaves already satisfy the heap property because they have no children. Therefore they need no sifting.
...
> Your answer: After swapping the root with the last unsorted element. The sorted tail is already finalized. It only the root may violate heap order
...
> Your answer: Input order does not strongly affect heapsort because its operations depend mainly on heap structure, not presortedness.
...
> Your answer: Most inputs are efficiently handled by quicksort partitioning, while heapsort only appears as a safety mechanism to avoid worst case in O(n^2)
