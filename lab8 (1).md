...
Sorted insertion   — height: 14
Shuffled insertion — height: 3

> Answer: r = 14/3 So the sorted tree is about 4.7 times taller than the shuffled tree
...
> Answer: This output is not affected by the insertion order. Regardless of whether the BST was built using sorted insertion or shuffled insertion, an in-order traversal of a BST always visits the keys in ascending order.
...
> Answer: isEnd = true because "car" is a complete word in the dictionary. child pointers that continue to other letters because longer words beginning with "car" also exist
...
> Answer: that node has isEnd = false because "ap" itself was never inserted as a full word. The words continuing from that node are "apple" "application" "apply" "apt" So trie_hasPrefix("ap") returns true because the path exists. But trie_search("ap") returns false because the final node is not marked as a complete word.

**[OBSERVE 5]**
Look at the four words sharing the prefix `"app"`: `apple`, `application`, `apply`, `apt`. 

- How many trie nodes do all four share before their paths diverge? They share 2 trie nodes before diverging.
- How many nodes would a string BST use to store the same four words? A string BST would store each whole word separately as its own node.
...
>Answer: A BST can use less memory when the stored strings share very few prefixes. If words diverge almost immediately the trie cannot share many nodes, so it creates nearly one node per character.
...
**[OBSERVE 7]**
After every removal, is the BST property preserved?
> Answer: Yes, the BST property is preserved after every removal.

 How does the in-order output confirm this without drawing the tree?

> Answer: The in-order traversal confirms this because an in-order traversal of a valid BST always produces sorted output.
...
1. Which case fires? This is the one-child case.
2. What pointer is returned to replace node 30? The remove function returns the pointer to node 40.
3. What happens to node 35? when 40 moves up, its entire subtree moves with it.
...
> Answer: Just before remove(50) is called, the tree has 50 as the root, with 40 and 70 as its children, 35 as the left child of 40, and 60 and 80 as the children of 70.
...
> Answer: Choosing the in-order successor or predecessor does not affect the correctness of the BST because both methods preserve the BST property. However, over many insertions and deletions, the choice can affect the shape and height of the tree.
...
> Answer: The trie completed the 500,000 exact searches in 281,759 microseconds, while the BST took 759,382 microseconds.
...
> Answer: A randomly built BST is not perfectly balanced because random insertions do not guarantee that nodes are evenly distributed across the tree. Even with random data, some branches become deeper than others, causing the height to grow larger than the ideal value of about 15.
