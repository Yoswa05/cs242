...
the ratio of BST to AVL is 3. O(n)/log_2(n) where n = 1000
...
All four rotation cases place the median value, 20, at the root because rotations reorganize the tree to maintain the binary search tree order while minimizing height. The median naturally becomes the root because it leaves one smaller value on the left and one larger value on the right, creating a balanced subtree.
---
...
During the first seven insertions, the values are added in a way that keeps the left and right subtrees of the root nearly equal in height. Because the balance factor of the root never exceeds ±1, the AVL tree remains balanced and no rotations are required.
...
1. What value moves **up** to become the new root? When the root node [10∣20∣30] is split, the middle value, 20, moves up and becomes the new root.
2. What two nodes remain as children? The values 10 and 30 remain in separate child nodes.
3. Where does `40` finally land? After the split, 40 is inserted into the right child, resulting in the node [30∣40].
...
Why doesn't sorted input degenerate a 2-3-4 tree the way it degenerates a plain BST? What structural property prevents it? Sorted input does not degenerate a 2-3-4 tree because nodes can store multiple keys and are split whenever they become full. This keeps the tree balanced automatically and prevents the long chain structure that occurs in a plain BST.
...
The timing ratio is smaller than the height ratio because insertion involves more than comparisons. Memory access, pointer updates, and cache effects also contribute to runtime. Deep BST traversals cause more cache misses, but the relationship between height and execution time is not perfectly linear, so the timing ratio is lower than the height ratio.
...
The results show that a plain BST can be a reasonable choice when input is random because its height tends to remain close to logarithmic on average. However, it does not guarantee good performance, since certain insertion orders can still produce a highly unbalanced tree with worst case O(n) operations.
...
A std::map node contains extra information such as a parent pointer and a color bit for maintaining Red-Black tree properties. These additional fields increase memory usage and can reduce cache efficiency because fewer nodes fit into cache lines, making searches and insertions slightly slower than in the simpler AVL node structure.
...
For a database index stored on disk, I would choose a 2-3-4 tree because it has a much smaller height than binary trees. With 10 million keys, fewer levels mean fewer disk reads, and each disk read costs about 5 ms. AVL and Red-Black trees would require more node accesses per search, increasing total disk latency. The 2-3-4 tree minimizes disk I/O and is therefore the most efficient choice for large on-disk databases and range queries.