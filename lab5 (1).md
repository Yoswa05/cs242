...
> Your answer: there are 11 slots, 5 are being used. so 11-5 is 6 empty slots. it occured 4 collision.
...
> Your answer: Prepending does not affect correctness. the hash table will still find all keys regardless of order.
...
> Your answer: 0 nodes examined. Expected probes ≈ 1.14 nodes
...
> Your answer: Chaining does not require this invariant, which is why it continues to function.
...
> Your answer: The longest chain grows very slowly as n increases. Which is much smaller than n, but still larger than the average (which is λ ≈ 1).
...
|-----------------------|--------------------------|--------|--------|
| carol (before delete) | 2                        | 1      | Found  |
| carol (after delete)  | 2 3 4                    | 3      | Found  |
| alice                 | 10                       | 1      | Found  |
| grace                 | 1 2                      | 2      | Found  |
| zara                  | ?                        | 0-2    |Not Found|
...
> Your answer: It extends the cluster by adding another occupied slot at its end
...
> Your answer: The search would fail because Linear probing relies on the rule “Stop only when you see EMPTY”
...
> Your answer: As tombstones accumulate, more slots behave like “blockers” for stopping conditions, Rehearsing rebuilds the table and removes all tombstones.
...
> Your answer: For open addressing, every key must live in the array itself, each slot holds exactly one element.
...
| ~0.50           | ~1.25    | ~2.00`         | ~1.35          |
| ~0.69           | ~1.35    | ~3.00          | ~1.70          |
| ~0.89           | ~1.45    | ~7.00 - 10.00  | ~2.50 - 3.50   |
...
> Your answer: keys spread out into multiple independent probe paths.
...
> Your answer: linear probing exceeds 3 probes at λ ≈ 0.7.
...
> Your answer: The experiment confirms the theory, but real hashing behavior always introduces noise and clustering effects.
