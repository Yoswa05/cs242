...
> Your answer: Different implementations can satisfy the same ADT as long as they follow the same rules and produce the same results.
...
> Your answer: The top index in the array stack represents the position of the current top element in the array.
...
> Your answer: Pushing the 17th item would cause a stack overflow, in the linked stack there is no fixed limit and It would continue growing normally. So if maximum stack depth is unknown, a linked stack is safer, while an array stack is better when size is predictable.
...
|-------------------|--------|------------------------|
| `({[]})`          | VALID  | it looks for its pair so it mactches.|
| `({[}])`          | INVALID| '[' does not match '}'|
| `((())`           | INVALID| Since there is no pair for '(' is invalid|
| `hello(world[!])` | VALID  | |
| *(empty string)*  | VALID  | |
...
> Your answer: The program fails at the character }, The stack top at that moment is '['. But } is trying to match with '{', not '['
...
> Your answer: The program ignores all non-bracket characters like letters, numbers, and symbols.

**Q8.** The empty string input reports VALID. Is this the right answer? Justify why an empty string is considered balanced.

> Your answer: An empty string is considered balanced because there are no unmatched brackets and there are no violations of the rules.
...
|-------------|-------------------------------|------------------|
| Initial     | *(empty)*                     |                — |
| enqueue(10) |front=0 back=1 cnt=1           |                — |
| enqueue(20) |front=0 back=2 cnt=2           |                — |
| enqueue(30) |front=0 back=3 cnt=3           |                — |
| dequeue     |front=1 back=3 cnt=2           |linked=10  array=10|
| enqueue(40) |front=1 back=4 cnt=3           |                — |
| dequeue     |front=2 back=4 cnt=2           |linked=20  array=20|
| dequeue     |front=3 back=4 cnt=1           |linked=30  array=30|
| enqueue(50) |front=3 back=5 cnt=2           |               — |
| enqueue(60) |front=3 back=6 cnt=3           |               — |
| enqueue(70) |front=3 back=7 cnt=4           |               — |
| dequeue     |front=4 back=7 cnt=3           |linked=40  array=40|
| dequeue     |front=5 back=7 cnt=2           |linked=50  array=50|
| dequeue     |front=6 back=7 cnt=1           |linked=60  array=60|
| dequeue     |front=7 back=7 cnt=0           |linked=70  array=70|
...
| Initial     | 0             | 0            | 0     |
| enqueue(10) | 10            | 0            | 1     |
| enqueue(20) | 10 20         | 20           | 2     |
| enqueue(30) | 10 20         | 30           | 3     |
| dequeue     | 20            | 30           | 2     |
| enqueue(40) | 20 30         | 40           | 3     |
| dequeue     | 30            | 40           | 2     |
| dequeue     | 0             | 40           | 1     |
| enqueue(50) | 40            | 50           | 2     |
| enqueue(60) | 40 50         | 60           | 3     |
| enqueue(70) | 40 50 60      | 70           | 4     |
| dequeue     | 50 60         | 70           | 3     |
| dequeue     | 60            | 70           | 2     |
| dequeue     | 0             | 70           | 1     |
| dequeue     | 0             | 0            | 0     |
...
> Your answer: Yes — after every operation, both the linked queue and array queue contain the same elements in the same order (front → back).
...
> Your answer: It simply incremented the front index (from 0 to 1). The element at index 0 is logically removed, even though it’s still physically in memory
...
> Your answer: if we use 'front == back' this could mean that the queue is empty 'OR' the queue is completely full (wrapped around). cnt (or an isFull flag) ensures we can correctly distinguish between full and empty states in a circular queue.
...
|-----------|-----------------|-----------------|-------|
| 100,000   | 11533           | 14503           |  0.80x|
| 500,000   | 56063           | 36236           |  1.55x|
| 1,000,000 | 39465           | 55272           |  0.71x|
| 5,000,000 | 219583          | 280466          |  0.78x|

### Observation Table 4b — Queue Performance (μs)

| n         | Linked enq+deq | Array enq+deq | Ratio |
|-----------|----------------|---------------|-------|
| 100,000   |            3372|          15291|  0.22x|
| 500,000   |           16887|          21839|  0.77x|
| 1,000,000 |           34491|          30469|  1.13x|
| 5,000,000 |          179186|         101026|  1.77x|
...
> Your answer: Yes, this is roughly consistent with O(n) total work, since runtime increases as n increases, even if not perfectly due to system factors (cache, memory allocation, etc.).
...
> Your answer: Arrays are faster because they are more memory-efficient and cache-friendly
...
> Your answer: Without reserve, many hidden reallocations happen, slowing the program down significantly.
...
> Your answer: Use arrays when size is predictable and performance is critical
...
> Your answer: If a recursive function has no base case, it keeps calling itself indefinitely.
