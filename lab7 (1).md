...
|-----------------|-------|---------------|
| mat[0][1]       | 4     | Edge between 0 and 1 with weight 4|
| mat[1][0]       | 4     | Same undirected edge stored symmetrically|
| mat[0][3]       | 0     | No edge between 0 and 3|
| mat[3][5]       | 7     | Edge between 3 and 5 with weight 7|
...
> Your answer: For V=100 and E=150, the adjacency list is much smaller because the graph is sparse. The list uses about 4,800 bytes, while the matrix uses 40,000 bytes.

For V=100 and E=4950, the adjacency matrix becomes smaller because the graph is dense. The list uses about 81,600 bytes, while the matrix still uses 40,000 bytes.

The crossover happens when the number of edges E becomes large enough that storing all neighbor pairs in the list approaches the fixed V^2 storage cost of the matrix.
16E + 24V ≈ 4V^2

which simplifies approximately to
E≈ V^2/4
So adjacency lists are better for sparse graphs, while adjacency matrices become better for very dense graphs.
...
> Your answer: One advantage of representing the 8-puzzle state as a tuple of 9 numbers is that tuples are immutable and hashable, which makes them efficient for use in sets and dictionaries. This is very important in search algorithms because we often need to quickly check whether a state has already been visited. Using tuples also reduces memory overhead compared to nested 2D arrays and makes copying states simpler and faster.
...
> Your answer: BFS guarantees an optimal solution when all actions have the same cost because it always explores the shallowest nodes first. This means it checks every possible path of length 1 before any path of length 2, every path of length 2 before any path of length 3, and so on.
...
> Your answer: The frontier grows rapidly in BFS because the algorithm stores every generated node at the current depth before moving deeper into the search tree. In problems like the 8-puzzle, each state can produce multiple child states, causing the number of nodes to increase exponentially as depth increases.
...
> Your answer: DFS uses significantly less memory than BFS because DFS only stores the current path being explored along with a small number of unexplored sibling nodes. In contrast, BFS must store every node at the current depth level, which can grow exponentially as the search gets deeper.
...
> Your answer: DFS is not guaranteed to find the optimal solution because it explores one branch as deeply as possible before considering alternative paths. If the first branch explored contains a solution, DFS immediately returns it even if a much shorter solution exists elsewhere in the search tree. 
...
> Your answer: Without a depth limit, DFS could become trapped exploring an infinitely deep branch or repeatedly cycling through states without ever reaching the goal. In problems with loops or extremely large search spaces, the algorithm might never terminate even if a solution exists elsewhere.
...
> Your answer: UCS expands the node with the lowest cumulative path cost from the start state. It uses a priority queue so that the cheapest path discovered so far is always explored first. This guarantees an optimal solution because UCS will always reach the goal through the minimum-cost path before exploring more expensive alternatives, assuming all step costs are nonnegative. BFS, in contrast, expands nodes based only on depth (the number of steps taken), not the actual cost of those steps. BFS guarantees optimality only when every action has the same cost.
...
> Your answer: UCS may take a longer route instead of moving directly toward the goal because it tries to minimize total path cost rather than the number of moves. In this grid, cells with cost 5 represent difficult terrain, while cells with cost 1 are cheap to traverse. A direct route through the middle of the grid would require entering several high-cost cells, making the total cost much larger.
...
> Your answer: UCS performs identically to BFS when every action has the same cost. In that case, minimizing total path cost is equivalent to minimizing the number of steps. Examples include simple maze solving, unweighted graphs, or the standard 8-puzzle where every move costs 1.
...
| DFS            | 0.523     |
| Dijkstra       | 1.376     |
| Bellman-Ford   | 202.480   |
| Topo Sort      | 0.116     |
| Prim MST       | 2.163     |
| Floyd-Warshall | 3769.417  |
...
> Your answer:Bellman-Ford becomes significantly slower in medium and especially dense graphs because its running time grows proportionally to O(VE) meaning it repeatedly relaxes every edge for each vertex.

Dijkstra with a priority queue grows approximately as O((V+E)logV) which scales much better as the number of edges increases.

In dense graphs, E becomes very large, so Bellman-Ford’s repeated edge processing causes its runtime to increase rapidly, while Dijkstra remains relatively efficient. This explains why Bellman-Ford jumps from about 52 ms to over 1600 ms as density increases.
...
> Your answer: I would choose Dijkstra’s algorithm because all road travel times are positive, and Dijkstra is specifically optimized for shortest-path problems with nonnegative edge weights. its complexity is O((V+E)logV)
